...
public class Compute {
	...
	static Compute ms = new Compute(); // 当前对象的引用
	...
	public String[] yusu(final String s)// 分析语速的同时将中缀变后缀
	{
		...
		for (int i = 0; i < s.length(); i++) {
			...
			if ((s.charAt(i) < '0' || s.charAt(i) > '9') && s.charAt(i) != '.')// 如果不是数字或小数点
																				// 换句话说
																				// 就是+-*/%()中的一个
			{
				...
				} else if (s.charAt(i) == '(')// 若是"("符号，则直接入栈 并设置其优先级最低
				{
					ms.push("(", stack1);
				} else if (s.charAt(i) == ')')// 如果字符是个")"，在遇见"("前，弹出所有操作符，然后把它们添加到list
				{
				...
				} else if (ms.properties(s.charAt(i) + "") > ms.properties(ms
						.seek(stack1)))// 剩下的情况只有+-*/和%了 如果优先级比栈顶元素的高则入栈
				{
					ms.push(s.charAt(i) + "", stack1);
				} else// 优先级小的则栈顶元素出栈进入list 一直到比栈顶优先级大为止 然后它再进栈
				{
					System.out.println("低级符号等待");
					while (ms.properties(s.charAt(i) + "") <= ms.properties(ms
							.seek(stack1))) {
						list[j] = ms.pop(stack1);
						j++;
						if (stack1[0] == null)
	...
	public static void main(String[] args) {
		...
		String s = "(3.5)+51*7-(4+6)*2";// 3.5 5 7 * + 4 6 + 2 * ( - (
